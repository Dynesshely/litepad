...
const messages: Record<LocaleId, Record<string, string>> = {
  'zh-CN': {
    ...
    'cmd.open': '命令菜单',
    'cmd.openHint': '命令菜单（Ctrl/⌘+Shift+P 或 F1）',
    'cmd.placeholder': '输入命令名称，例如「排序」「Base64」…',
    'cmd.empty': '没有匹配的命令',
    'cmd.hint': '↑↓ 选择 · Enter 执行 · Esc 关闭',
    'cmd.applied': '已执行：{title}',
    'cmd.error': '执行失败：{title}（{msg}）',
    'cmd.noEditor': '编辑器尚未就绪',
    'cmd.group.lines': '行操作',
    'cmd.group.case': '大小写与命名',
    'cmd.group.data': '编码与数据',
    'cmd.group.text': '文本',
    'cmd.group.nav': '导航与信息',
    'cmd.lines.join': '多行转单行（输入连接符）',
    'cmd.lines.split': '单行转多行（以选中文本为分隔符）',
    'cmd.lines.splitAll': '整篇按分隔符拆分为多行',
    'cmd.lines.sortAsc': '行排序：升序（数字感知）',
    'cmd.lines.sortDesc': '行排序：降序',
    'cmd.lines.dedupe': '行去重（保留首次出现）',
    'cmd.lines.reverse': '反转行顺序',
    'cmd.lines.removeEmpty': '删除空行',
    'cmd.lines.trim': '去除每行首尾空白',
    'cmd.lines.trimTrailing': '删除行尾空白',
    'cmd.lines.number': '每行添加行号',
    'cmd.lines.quote': '每行加双引号',
    'cmd.lines.comment': '切换行注释（// ）',
    'cmd.case.upper': '转为大写',
    'cmd.case.lower': '转为小写',
    'cmd.case.toggle': '大小写互换',
    'cmd.case.kebab': '命名转为 kebab-case',
    'cmd.case.snake': '命名转为 snake_case',
    'cmd.case.camel': '命名转为 camelCase',
    'cmd.base64.encode': 'Base64 编码',
    'cmd.base64.decode': 'Base64 解码',
    'cmd.url.encode': 'URL 编码（encodeURIComponent）',
    'cmd.url.decode': 'URL 解码',
    'cmd.json.format': 'JSON 美化（2 空格缩进）',
    'cmd.json.minify': 'JSON 压缩为单行',
    'cmd.json.escape': '转义为 JSON 字符串',
    'cmd.json.unescape': '反转义 JSON 字符串',
    'cmd.text.reverseChars': '反转字符顺序',
    'cmd.text.markdownQuote': '每行加 Markdown 引用（> ）',
    'cmd.text.collapseSpaces': '折叠连续空格',
    'cmd.text.stats': '文本统计',
    'cmd.nav.goToLine': '跳转到行',
    'cmd.prompt.joiner.title': '多行转单行',
    'cmd.prompt.joiner.label': '连接符（支持 \\n 表示换行）',
    'cmd.prompt.joiner.placeholder': '例如 , 或 \\n',
    'cmd.prompt.splitter.title': '单行转多行',
    'cmd.prompt.splitter.label': '分隔符',
    'cmd.prompt.splitter.placeholder': '例如 , 或 |',
    'cmd.prompt.lineNo.label': '行号',
    'cmd.prompt.lineNo.placeholder': '1',
    'cmd.stats.titleSelection': '文本统计（选区）',
    'cmd.stats.titleAll': '文本统计（全文）',
    'cmd.stats.chars': '字符数',
    'cmd.stats.charsNoSpaces': '字符数（不含空白）',
    'cmd.stats.words': '词数',
    'cmd.stats.lines': '行数',
    'cmd.stats.bytes': 'UTF-8 字节数',
    'dialog.ok': '确定',
    'dialog.cancel': '取消',
    'dialog.close': '关闭',
    ...
    'settings.noResults': '没有匹配的设置项',
    'settings.searchHint': '↑↓ 选择 · Enter 跳转 · Esc 关闭',
    ...
    'settings.appearance.backgroundHint': '图片保存在本机 IndexedDB，不会上传到任何服务器。',
    'settings.appearance.choose': '选择图片…',
    'settings.appearance.clear': '清除背景',
    'settings.appearance.none': '未设置',
    'settings.appearance.blur': '背景模糊度',
    'settings.appearance.blurHint': '模糊壁纸能让正文更易读（0 表示不模糊）。',
    'settings.appearance.opacity': '图片可见度',
    'settings.appearance.opacityHint': '数值越低，遮罩越强、正文越清晰。',
    'settings.appearance.editorTint': '编辑区保留底色',
    'settings.appearance.editorTintHint': '壁纸下给正文留一层淡底色，照片再花也读得清；关闭则编辑区完全透出壁纸。',
    'settings.toggle.on': '开',
    'settings.toggle.off': '关',
    'settings.data.import': '导入备份',
    'settings.data.importHint': '从 JSON 备份文件合并导入草稿（同一草稿保留较新的那份）。',
    'settings.data.backup': '备份全部',
    'settings.data.backupHint': '把全部草稿（含编码与排序）导出为一个 JSON 文件。',
    'settings.data.usage': '存储占用',
    'settings.data.drafts': '草稿数量',
    'settings.data.draftsValue': '{n} 篇',
    'settings.imageInvalid': '请选择图片文件',
    'settings.imageTooLarge': '图片太大了（上限 {size}）',
    'settings.imageSaved': '背景图片已保存到本机',
    'settings.imageCleared': '已清除背景图片',
    'settings.unavailable': 'IndexedDB 不可用，无法保存图片',
  ...
  'en-US': {
    ...
    'cmd.openHint': 'Command palette (Ctrl/⌘+Shift+P or F1)',
    'cmd.placeholder': 'Type a command, e.g. “sort”, “base64”…',
    'cmd.empty': 'No matching commands',
    'cmd.hint': '↑↓ navigate · Enter run · Esc close',
    'cmd.applied': 'Ran: {title}',
    'cmd.error': 'Failed: {title} ({msg})',
    'cmd.noEditor': 'Editor is not ready yet',
    'cmd.group.lines': 'Lines',
    'cmd.group.case': 'Case & naming',
    'cmd.group.data': 'Encoding & data',
    'cmd.group.text': 'Text',
    'cmd.group.nav': 'Navigate & info',
    'cmd.lines.join': 'Join lines into one (asks for a joiner)',
    'cmd.lines.split': 'Split line into many (selection = separator)',
    'cmd.lines.splitAll': 'Split whole document by a separator',
    'cmd.lines.sortAsc': 'Sort lines: ascending (numeric-aware)',
    'cmd.lines.sortDesc': 'Sort lines: descending',
    'cmd.lines.dedupe': 'Remove duplicate lines',
    'cmd.lines.reverse': 'Reverse line order',
    'cmd.lines.removeEmpty': 'Remove empty lines',
    'cmd.lines.trim': 'Trim each line',
    'cmd.lines.trimTrailing': 'Trim trailing whitespace',
    'cmd.lines.number': 'Number each line',
    'cmd.lines.quote': 'Wrap each line in double quotes',
    'cmd.lines.comment': 'Toggle line comment (// )',
    'cmd.case.upper': 'To UPPERCASE',
    'cmd.case.lower': 'To lowercase',
    'cmd.case.toggle': 'Toggle case',
    'cmd.case.kebab': 'Naming → kebab-case',
    'cmd.case.snake': 'Naming → snake_case',
    'cmd.case.camel': 'Naming → camelCase',
    'cmd.base64.encode': 'Base64 encode',
    'cmd.base64.decode': 'Base64 decode',
    'cmd.url.encode': 'URL encode (encodeURIComponent)',
    'cmd.url.decode': 'URL decode',
    'cmd.json.format': 'Format JSON (2-space indent)',
    'cmd.json.minify': 'Minify JSON to one line',
    'cmd.json.escape': 'Escape as a JSON string',
    'cmd.json.unescape': 'Unescape a JSON string',
    'cmd.text.reverseChars': 'Reverse characters',
    'cmd.text.markdownQuote': 'Prefix each line with “> ”',
    'cmd.text.collapseSpaces': 'Collapse repeated spaces',
    'cmd.text.stats': 'Text statistics',
    'cmd.nav.goToLine': 'Go to line',
    'cmd.prompt.joiner.title': 'Join lines',
    'cmd.prompt.joiner.label': 'Joiner (use \\n for a newline)',
    'cmd.prompt.joiner.placeholder': 'e.g. , or \\n',
    'cmd.prompt.splitter.title': 'Split into lines',
    'cmd.prompt.splitter.label': 'Separator',
    'cmd.prompt.splitter.placeholder': 'e.g. , or |',
    'cmd.prompt.lineNo.label': 'Line number',
    'cmd.prompt.lineNo.placeholder': '1',
    'cmd.stats.titleSelection': 'Text statistics (selection)',
    'cmd.stats.titleAll': 'Text statistics (whole document)',
    'cmd.stats.chars': 'Characters',
    'cmd.stats.charsNoSpaces': 'Characters (no whitespace)',
    'cmd.stats.words': 'Words',
    'cmd.stats.lines': 'Lines',
    'cmd.stats.bytes': 'UTF-8 bytes',
    'dialog.ok': 'OK',
    ...
    'settings.searchHint': '↑↓ select · Enter jump · Esc close',
    ...
    'settings.appearance.backgroundHint': 'The image is stored locally in IndexedDB and never uploaded.',
    'settings.appearance.choose': 'Choose image…',
    'settings.appearance.clear': 'Remove background',
    'settings.appearance.none': 'Not set',
    'settings.appearance.blur': 'Background blur',
    'settings.appearance.blurHint': 'Blurring the wallpaper keeps text readable (0 = sharp).',
    'settings.appearance.opacity': 'Image visibility',
    'settings.appearance.opacityHint': 'Lower values dim the image so text stays readable.',
    'settings.appearance.editorTint': 'Tint behind the editor',
    'settings.appearance.editorTintHint': 'Keeps a faint theme tint behind the text so busy photos stay readable; off means the editor is fully transparent.',
    'settings.toggle.on': 'On',
    'settings.toggle.off': 'Off',
    'settings.data.import': 'Import backup',
    'settings.data.importHint': 'Merge drafts from a JSON backup (newer copy wins per draft).',
    'settings.data.backup': 'Back up all',
    'settings.data.backupHint': 'Export every draft (including encoding and order) as one JSON file.',
    'settings.data.usage': 'Origin usage',
    'settings.data.drafts': 'Drafts',
    'settings.data.draftsValue': '{n}',
    'settings.imageInvalid': 'Please choose an image file',
    'settings.imageTooLarge': 'Image is too large (limit {size})',
    'settings.imageSaved': 'Background image saved locally',
    'settings.imageCleared': 'Background image removed',
    'settings.unavailable': 'IndexedDB is unavailable — cannot store the image',
